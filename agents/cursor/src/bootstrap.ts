...
const STATE_FILE = resolve(STATE_DIR, "cursor-state.json");
const JOIN_REQUEST_FILE = resolve(STATE_DIR, "cursor-join-request.json");
...
function loadState(): CursorState | null {
  if (!existsSync(STATE_FILE)) return null;
  try {
    return JSON.parse(readFileSync(STATE_FILE, "utf8")) as CursorState;
  } catch {
    return null;
  }
}
...
export async function bootstrapAgent(opts: {
  ...
  const waitForJoinApproval = async (requestId: string, requestSecret: string): Promise<string> => {
    ...
    for (;;) {
      ...
      console.log(
        `[cursor] waiting for admin approval on join request ${requestId} (polling every ${pollMs}ms)`,
      );
      await new Promise((resolve) => setTimeout(resolve, pollMs));
    }
  };

  const provider = (opts.provider ?? "cursor-agent").trim() || "cursor-agent";
