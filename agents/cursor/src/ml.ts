...
export class MessageLayerClient {
  ...
  private async waitForPermissionDecision(
    ...
    const timeoutMs =
      opts.timeoutMs ??
      Number(process.env.CURSOR_PERMISSION_TIMEOUT_MS ?? "300000");
    const pollIntervalMs =
      opts.pollIntervalMs ??
      Number(process.env.CURSOR_PERMISSION_POLL_MS ?? "2000");
  ...
  > {
    ...
    if (status !== 200) {
      throw new Error(`listStreamEvents failed ${status}: ${JSON.stringify(body)}`);
    }
