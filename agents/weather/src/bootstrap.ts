...
const STATE_FILE = resolve(STATE_DIR, "weather-state.json");
const JOIN_REQUEST_FILE = resolve(STATE_DIR, "weather-join-request.json");
...
function loadState(): WeatherState | null {
  if (!existsSync(STATE_FILE)) return null;
  try {
    return JSON.parse(readFileSync(STATE_FILE, "utf8")) as WeatherState;
  } catch {
    return null;
  }
}

function saveState(state: WeatherState): void {
...
export async function bootstrapAgent(opts: {
  ...
  const resolveWorkspace = async (): Promise<{ orgId: string; hasWorkspace: boolean }> => {
    ...
    if (!setup.ok) throw new Error(`failed to load workspace setup info (${setup.status})`);
  ...
  const waitForJoinApproval = async (requestId: string, requestSecret: string): Promise<string> => {
    ...
    for (;;) {
      ...
      if (status.status === "approved" && status.actorId) return status.actorId;
      ...
      console.log(
        `[weather] waiting for admin approval on join request ${requestId} (polling every ${pollMs}ms)`,
      );
